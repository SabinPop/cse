<?php  function lcr57f1ac70d787ehbch($cx, $ch, $vars, $op, $inverted, $cb = null, $else = null) {
  ...
  if ($isBlock) {
   $options['fn'] = function ($context = '_NO_INPUT_HERE_', $data = null) use ($cx, &$op, $cb, $options, $vars) {
    ...
    if (($context === '_NO_INPUT_HERE_') || ($context === $op)) {
     $ret = $cb($cx, is_array($ex) ? lcr57f1ac70d787em($cx, $op, $ex) : $op);
    } else {
     $cx['scopes'][] = $op;
     $ret = $cb($cx, is_array($ex) ? lcr57f1ac70d787em($cx, $context, $ex) : $context);
     array_pop($cx['scopes']);
    }
    if (isset($data['data'])) {
  ...
  if($e !== null) {
   lcr57f1ac70d787eerr($cx, $e);
  }

  return $r;
 }
 function lcr57f1ac70d787eencq($cx, $var) {
  if ($var instanceof LS) {
   return (string)$var;
  }

  return str_replace(array('=', '`', '&#039;'), array('&#x3D;', '&#x60;', '&#x27;'), htmlentities(lcr57f1ac70d787eraw($cx, $var), ENT_QUOTES, 'UTF-8'));
 }
 function lcr57f1ac70d787ev($cx, $in, $base, $path, $args = null) {
  ...
  if ($args) {
   lcr57f1ac70d787eerr($cx, 'Can not find helper or lambda: "' . implode('.', $path) . '" !');
  }
 }
 function lcr57f1ac70d787esec($cx, $v, $bp, $in, $each, $cb, $else = null) {
  $push = ($in !== $v) || $each;
  ...
  if (($loop && $isAry) || $isTrav) {
   ...
   foreach ($v as $index => $raw) {
    ...
    if (isset($bp[0])) {
     $raw = lcr57f1ac70d787em($cx, $raw, array($bp[0] => $raw));
    }
    if (isset($bp[1])) {
     $raw = lcr57f1ac70d787em($cx, $raw, array($bp[1] => $cx['sp_vars']['index']));
    }
    $ret[] = $cb($cx, $raw);
   }
   if ($cx['flags']['spvar']) {
 ...
 function lcr57f1ac70d787eifvar($cx, $v, $zero) {
  return ($v !== null) && ($v !== false) && ($zero || ($v !== 0) && ($v !== 0.0)) && ($v !== '') && (is_array($v) ? (count($v) > 0) : true);
 }
 function lcr57f1ac70d787ep($cx, $p, $v, $pid, $sp = '') {
  if ($p === '@partial-block') {
   $p = "$p" . ($pid > 0 ? $pid : $cx['partialid']);
  }

  if (!isset($cx['partials'][$p])) {
   lcr57f1ac70d787eerr($cx, "Can not find partial named as '$p' !!");
   return '';
  }

  $cx['partialid'] = $pid;

  return call_user_func($cx['partials'][$p], $cx, lcr57f1ac70d787em($cx, $v[0][0], $v[1]), $sp);
 }
 function lcr57f1ac70d787em($cx, $a, $b) {
  if (is_array($b)) {
   if ($a === null) {
    return $b;
   } else if (is_array($a)) {
 ...
 function lcr57f1ac70d787eerr($cx, $err) {
 ...
 function lcr57f1ac70d787eraw($cx, $v) {
  if ($v === true) {
   if ($cx['flags']['jstrue']) {
    return 'true';
   }
...
',$sp,'
',$sp,'    <form class="search" action="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'search_url', array(array(),array()), 'encq', $in)),'" autocomplete="off">
',$sp,'        <input type="search" name="s" placeholder="Search" class="form-control">
',$sp,'    </form>
',$sp,'
',$sp,'    <nav>
',$sp,'',lcr57f1ac70d787ehbch($cx, 'navigation', array(array(),array()), $in, false, function($cx, $in)use($sp){echo '',lcr57f1ac70d787esec($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('items')), null, $in, true, function($cx, $in)use($sp){echo '                <a class="nav-',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('slug'))),' ';if (lcr57f1ac70d787eifvar($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('current')), false)){echo 'current';}else{echo '';}echo '" href="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'url', array(array(lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('link'))),array()), 'encq', $in)),'">
',$sp,'                    ',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('label'))),'
',$sp,'                </a>
',$sp,'';}),'';}),'    </nav>
',$sp,'
',$sp,'    <a class="feed" href="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'feed_url', array(array(),array()), 'encq', $in)),'">
',$sp,'        RSS Feed
',$sp,'    </a>
',$sp,'
...
',$sp,'<html>
',$sp,'<head>
',$sp,'    <title>',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ev($cx, $in, isset($cx['sp_vars']) ? $cx['sp_vars'] : null, array('meta','title'))),'</title>
',$sp,'    <meta name="description" content="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ev($cx, $in, isset($cx['sp_vars']) ? $cx['sp_vars'] : null, array('meta','description'))),'">
',$sp,'
...
',$sp,'
',$sp,'    ',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'postleaf_head', array(array(),array()), 'encq', $in)),'
',$sp,'
',$sp,'    <link rel="shortcut icon" href="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'url', array(array(lcr57f1ac70d787ev($cx, $in, isset($cx['sp_vars']) ? $cx['sp_vars'] : null, array('settings','favicon'))),array()), 'encq', $in)),'">
',$sp,'
...
',$sp,'
',$sp,'    <link rel="stylesheet" href="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'theme_url', array(array('css/theme.css'),array()), 'encq', $in)),'">
',$sp,'</head>
',$sp,'
',$sp,'<body class="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'body_class', array(array(),array()), 'encq', $in)),'">
',$sp,'
',$sp,'',lcr57f1ac70d787ep($cx, 'navigation', array(array($in),array()),0, '    '),'
',$sp,'    <div class="body-wrap">
',$sp,'
',$sp,'';if (lcr57f1ac70d787eifvar($cx, lcr57f1ac70d787ev($cx, $in, isset($cx['sp_vars']) ? $cx['sp_vars'] : null, array('settings','logo')), false)){echo '            <a href="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'url', array(array(),array()), 'encq', $in)),'" class="logo">
',$sp,'                <img src="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'url', array(array(lcr57f1ac70d787ev($cx, $in, isset($cx['sp_vars']) ? $cx['sp_vars'] : null, array('settings','logo'))),array()), 'encq', $in)),'" alt="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ev($cx, $in, isset($cx['sp_vars']) ? $cx['sp_vars'] : null, array('settings','title'))),'">
',$sp,'            </a>
',$sp,'';}else{echo '';}echo '
',$sp,'        <a href="#" class="nav-toggle">Menu</a>';return ob_get_clean();},
'loop' => function ($cx, $in, $sp) {ob_start();echo '',$sp,'<div class="loop">
',$sp,'
',$sp,'',lcr57f1ac70d787esec($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('posts')), null, $in, true, function($cx, $in)use($sp){echo '
',$sp,'        <article class="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'post_class', array(array(),array()), 'encq', $in)),'">
',$sp,'            <h2 class="title"><a href="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'post_url', array(array(),array()), 'encq', $in)),'">',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'title', array(array(),array()), 'encq', $in)),'</a></h2>
',$sp,'
',$sp,'            <div class="excerpt">
',$sp,'                ',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'excerpt', array(array(),array('words'=>'50')), 'encq', $in)),'&nbsp;<a class="more" href="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'post_url', array(array(),array()), 'encq', $in)),'">&hellip;</a>
',$sp,'            </div>
',$sp,'
',$sp,'            <div class="meta">
',$sp,'',lcr57f1ac70d787ehbch($cx, 'author', array(array(),array()), $in, false, function($cx, $in)use($sp){echo '                    ';if (lcr57f1ac70d787eifvar($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('avatar')), false)){echo '<img class="avatar" src="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'url', array(array(lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('avatar'))),array()), 'encq', $in)),'" alt="Avatar">';}else{echo '';}echo '
',$sp,'                    <a class="author" href="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'author_url', array(array(),array()), 'encq', $in)),'">',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('name'))),'</a>
',$sp,'';}),'
',$sp,'                <span class="tags">
',$sp,'                    ',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'tags', array(array(),array('before'=>'on ','and'=>' and ')), 'encq', $in)),'
',$sp,'                </span>
',$sp,'
',$sp,'                <time class="date" datetime="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'date', array(array(),array('format'=>'%Y-%m-%d')), 'encq', $in)),'">
',$sp,'                    ',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'date', array(array(),array('format'=>'%B %e, %Y')), 'encq', $in)),'
',$sp,'                </time>
',$sp,'            </div>
',$sp,'        </article>
',$sp,'
',$sp,'';}),'
',$sp,'</div>';return ob_get_clean();},
'pagination' => function ($cx, $in, $sp) {ob_start();echo '',$sp,'',lcr57f1ac70d787esec($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('pagination')), null, $in, false, function($cx, $in)use($sp){echo '',lcr57f1ac70d787ehbch($cx, 'is', array(array(lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('total_pages')),'>',1),array()), $in, false, function($cx, $in)use($sp){echo '        <div class="post-pagination">
',$sp,'';if (lcr57f1ac70d787eifvar($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('previous_page_url')), false)){echo '                <div class="previous">
',$sp,'                    <a href="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('previous_page_url'))),'">Newer Posts</a>
',$sp,'                </div>
',$sp,'';}else{echo '';}echo '
',$sp,'',lcr57f1ac70d787ehbch($cx, 'is', array(array(lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('total_pages')),'>',1),array()), $in, false, function($cx, $in)use($sp){echo '                <div class="current">
',$sp,'                    Page ',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'number', array(array(lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('current_page'))),array()), 'encq', $in)),' of ',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'number', array(array(lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('total_pages'))),array()), 'encq', $in)),'
',$sp,'                </div>
',$sp,'';}),'
',$sp,'';if (lcr57f1ac70d787eifvar($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('next_page_url')), false)){echo '                <div class="next">
',$sp,'                    <a href="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('next_page_url'))),'">Older Posts</a>
',$sp,'                </div>
...
',$sp,'                        <a href="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'url', array(array(),array()), 'encq', $in)),'">',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ev($cx, $in, isset($cx['sp_vars']) ? $cx['sp_vars'] : null, array('settings','title'))),'</a> &copy;',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'date', array(array(),array('format'=>'%Y')), 'encq', $in)),'
',$sp,'                    </div>
...
',$sp,'    </div> 
',$sp,'
',$sp,'    ',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'postleaf_foot', array(array(),array()), 'encq', $in)),'
',$sp,'
...
',$sp,'
',$sp,'    <script src="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'theme_url', array(array('js/theme.js'),array()), 'encq', $in)),'"></script>
',$sp,'</body>
',$sp,'</html>';return ob_get_clean();});
    ...
    ob_start();echo '',lcr57f1ac70d787ep($cx, 'header', array(array($in),array()),0),'
',lcr57f1ac70d787ehbch($cx, 'author', array(array(),array()), $in, false, function($cx, $in) {echo '    <header class="cover" ';if (lcr57f1ac70d787eifvar($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('cover')), false)){echo 'style="background-image: url(\'',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'url', array(array(lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('cover'))),array()), 'encq', $in)),'\');"';}else{echo '';}echo '>
        <div class="content">
            <h1 class="title">',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('name'))),'</h1>
        </div>
    </header>
';}),'
<div class="container">
    <div class="row">
        <div class="col-md-8 push-md-2 col-sm-10 push-sm-1">
            <div class="author">
',lcr57f1ac70d787ehbch($cx, 'author', array(array(),array()), $in, false, function($cx, $in) {echo '';if (lcr57f1ac70d787eifvar($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('avatar')), false)){echo '                        <img class="avatar" src="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'url', array(array(lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('avatar'))),array()), 'encq', $in)),'" alt="Avatar">
';}else{echo '';}echo '
                    <div class="details">
';if (lcr57f1ac70d787eifvar($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('location')), false)){echo '                            <span class="location">
                                <span class="icon icon-location"></span>
                                ',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('location'))),'
                            </span>
';}else{echo '';}echo '
';if (lcr57f1ac70d787eifvar($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('website')), false)){echo '                            <span class="website">
                                <span class="icon icon-link"></span>
                                <a href="',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('website'))),'">',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ev($cx, $in, isset($in) ? $in : null, array('website'))),'</a>
                            </span>
';}else{echo '';}echo '                    </div>

                    <div class="bio">',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'bio', array(array(),array()), 'encq', $in)),'</div>

                    <div class="post-count">
                        ',lcr57f1ac70d787eencq($cx, lcr57f1ac70d787ehbch($cx, 'plural', array(array(lcr57f1ac70d787ev($cx, $in, isset($cx['scopes'][count($cx['scopes'])-1]) ? $cx['scopes'][count($cx['scopes'])-1] : null, array('pagination','total_items'))),array('none'=>'No posts','singular'=>'One post','plural'=>'% posts')), 'encq', $in)),'
                    </div>
';}),'            </div>

',lcr57f1ac70d787ep($cx, 'loop', array(array($in),array()),0, '            '),'
',lcr57f1ac70d787ep($cx, 'pagination', array(array($in),array()),0, '            '),'        </div>
    </div>
</div>

',lcr57f1ac70d787ep($cx, 'footer', array(array($in),array()),0),'';return ob_get_clean();
};